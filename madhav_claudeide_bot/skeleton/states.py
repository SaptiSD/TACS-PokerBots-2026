...
Fixed: handles empty opponent hand during redraw (opponent cards are unknown).
'''
    ...
    def proceed(self, action):
        ...
        if isinstance(action, RedrawAction):
            ...
            if self._is_valid_redraw_target(active, target_type, target_index):
                # Pad opponent hand with placeholders if unknown (opponent's cards are private).
                hands = [
                    list(self.hands[0]) if len(self.hands[0]) >= 2 else ['??', '??'],
                    list(self.hands[1]) if len(self.hands[1]) >= 2 else ['??', '??'],
                ]
