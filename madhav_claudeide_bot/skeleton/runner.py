...
Extended: calls bot.on_opponent_redraw() with the revealed old card.
'''
import argparse
import socket
...
class Runner():
    ...
    def _apply_action_clause(self, round_state, action_clause, active):
        ...
        if redraw_info is not None:
            ...
            # Notify bot about opponent redraw with the revealed old card.
            if actor != active and old_card is not None:
                self.pokerbot.on_opponent_redraw(target_type, target_index, old_card)
        ...
        for packet in self.receive():
            for clause in packet:
                ...
                if code == 'T':
                    ...
                    # Revealed old card from opponent's redraw.
                    actor = 1 - active
                    self._pending_redraw_old_card[actor] = clause[1:]
                elif code in ('F', 'C', 'K', 'R'):
                    round_state = self._apply_action_clause(round_state, clause, active)
                elif code == 'B':
